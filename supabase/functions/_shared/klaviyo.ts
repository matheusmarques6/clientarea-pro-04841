// ============================================================================
// KLAVIYO API CLIENT - Deno Version
// Adaptado do código TypeScript para Edge Functions
// ============================================================================
...
export async function fetchKlaviyoData(
  ...
  console.log('[Klaviyo] Period:', startDate, 'to', endDate)
  ...
  if (flowsRes.success && flowsRes.data?.data) {
    ...
    const baseFlows = todosFlows.map((flow: any) => ({
      id: flow.id,
    ...
    // Buscar receita e performance de cada flow
    if (metricaId && baseFlows.length > 0) {
      console.log('[Klaviyo] Fetching flow metrics...')
      const relatorios = await execQueue(baseFlows, 3, async (flowData: Flow) => {
        // Buscar receita
        const valuesBody = {
          data: {
            type: 'flow-values-report',
        ...
        // Buscar performance (email stats)
        const perfBody = {
          data: {
        ...
        const perfRes = await klaviyoRequest(apiKey, 'POST', '/flow-series-reports/', '', perfBody)
        if (perfRes.success && perfRes.data?.data?.attributes?.results?.[0]) {
          const perfStats = perfRes.data.data.attributes.results[0].statistics || {}
      ...
      flows.push(...relatorios)
    } else {
      flows.push(...baseFlows)
    }
  }

  console.log(`[Klaviyo] Processed flows: ${flows.length}`)
